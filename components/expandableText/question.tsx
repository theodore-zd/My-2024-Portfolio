...
export const Question = ({ title, desc, index, dark }:{title:string, desc:string, index:number, dark:boolean}) => {
  ...
  <motion.div
    className={
      'flex flex-col bg-theme-text-alt border-[1px] max-h-fit border-theme-text-main rounded-lg shadow-md mt-4 cursor-pointer' +
      (dark ? 'bg-theme-text-alt text-theme-text-main' : 'bg-theme-text-alt text-theme-dark ')
    }
    initial="hidden"
  ...
  >
    <div className={'flex mt-2 cursor-pointer ' + (animation ? 'mb-0' : 'mb-2')}>
      <div className='w-6 h-6 ml-4 mt-3.5'>
        <motion.div
          className='h-0.5 w-4 bg-theme-text-main absolute'
          initial="closed"
